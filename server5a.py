...
class CommanderGUI:
    def __init__(self, root):
        ...
        self.connected_tanks = {}    # Store tank information
                ...
                if "\n" in buffer:
                    ...
                    try:
                        ...
                        if location:
                            ...
                            logging.info(f"Sent acknowledgment to Tank {tank_id}")
                            
                            # Update tank location immediately
                            self.update_tank_location(tank_id, location)

                            # Broadcast location to all connected clients
                            self.broadcast_location(tank_id, location)
                        else:
    ...
    def broadcast_location(self, tank_id, location):
        """Broadcast location updates to all connected clients"""
        message = json.dumps({"tank_id": tank_id, "location": location})
        for client_conn in self.connected_clients.values():
            try:
                client_conn.sendall(message.encode() + b"\n")
            except Exception as e:
                logging.error(f"Failed to send location update to a client: {e}")
